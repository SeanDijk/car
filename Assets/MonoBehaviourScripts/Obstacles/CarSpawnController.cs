...
public class CarSpawnController : MonoBehaviour {
    ...
    public void turnOnSpawner(int i)
    {
        ...
        spawnObject.GetComponent<CarSpawner>().ToggleSpawner(true);
    }
    public void turnOffSpawner(int i)
    {
        i = i - 1;
        GameObject spawnObject = gameObjectSpawnerList[i];
        spawnObject.GetComponent<CarSpawner>().ToggleSpawner(false);
    ...
    void createSpawnLocations()
    {
        for (int i = 0; i < spawnLocations.Count;)
        {
            ...
            spawnPoint.AddComponent<CarSpawner>().carRouteID = spawnLocationRouteIds[i];
            spawnPoint.GetComponent<CarSpawner>().prefab = spawnTriggerPrefab[i];
            spawnPoint.transform.position = spawnLocations[i];
            spawnPoint.gameObject.name = "Spawnpoint";
