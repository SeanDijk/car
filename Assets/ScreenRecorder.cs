...
public class ScreenRecorder : ScriptableObject
{
    ...
    public string UniqueFilename(Camera cam)
    {
        ...
        // use width, height, and camera for unique file name
        var filename = string.Format("{0}/screen_{1}x{2}_{3}.{4}", folder, (int)rect.width, (int)rect.height, cam.ToString(), format.ToString().ToLower());

        // return unique filename
        return filename;
    }

    public void CaptureScreenshot(Camera cam)
    {
        ...
        string filename = UniqueFilename(cam);
